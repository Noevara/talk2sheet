...
Talk2Sheet 是一个面向 Excel / CSV 的开源表格对话分析框架。当前版本聚焦在：

- workbook 内的单 sheet 智能路由与分析
- 自然语言到结构化分析计划的转换
- 基于 pandas 的可执行分析链路
...
- 已支持：workbook 内单 sheet 选择、自动路由、继续追问、明细 / 汇总 / 排名 / 趋势 /基础图表 / 轻量预测
- 暂不支持：跨 sheet join、跨 sheet 联合聚合、多表关系建模、高级统计、因果推断、复杂预测工作流
...
- `P0`
  ...
  - 基础可观测性补齐
- `P1`
  - 文件存储抽象为 `services/storage/`
  - 前端按 feature 组织
  - CI 分层校验
- `P2`
  - 引入 semantic intent 层
  - 实现 `P2-2A`：workbook 内单 sheet 路由，而不是跨 sheet 联合执行
...
后端主代码位于 `apps/api/app/`，当前采用“按职责分包”的组织方式，而不是大而全的 DDD 重写。
...
这部分是 `P1-1` 的结果，目的是把“文件存储”从 spreadsheet 业务代码里抽离出来。
...
2. 澄清结果命中
3. 问题中显式指定 sheet
4. 手动切换 sheet 的 `sheet_override`
5. follow-up 继承上轮 sheet
6. 按 sheet 名 / 列名 / hints 自动评分
7. 候选冲突时发起澄清
8. 回退到请求 sheet
...
- `apps/api/app/services/spreadsheet/planning/`
  planner、heuristic / llm provider、follow-up planning、guardrails
- `intent_models.py`
  `AnalysisIntent` 结构
...
这部分是 `P2-1` 的核心成果。系统不再只靠一个松散的 `intent` 字符串，而是保留结构化语义，如：
...
- `apps/api/app/services/spreadsheet/analysis/`
  总调度层，负责把 routing 后的 dataframe 推入 planner / validation / execution / answer generation
...
- `apps/api/app/services/spreadsheet/conversation/`
  会话记忆、follow-up context、rule-based / llm-based summarizer、结果格式化
...
前端主代码位于 `apps/web/src/`，当前已经从单页大组件演进为 feature 化结构。
...
- `apps/web/src/features/conversation/composables/useConversation.ts`
  管理问题输入、消息列表、conversation id、clarification 追问、请求组装
...
- `apps/web/src/components/ConversationMessage.vue`
  答案卡片、sheet routing、execution disclosure、明细表、图表、structured answer
...
1. 在现有 single-sheet routing 之上，继续推进 `P2-2B` 级别的多 sheet 规划，而不是直接把执行层改成自由 join。
