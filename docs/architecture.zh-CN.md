...
- 面向 workbook 的单 sheet 路由
  ...
  - 当前已支持在同一 workbook 内定位目标 sheet，并在该 sheet 上完成分析
  - 当前仍不支持跨 sheet 联合执行
- 自然语言分析
  - 总行数、总量、均值、去重计数
  - Top N / ranking
...
- `apps/web/src/components/ConversationMessage.vue`
  答案卡片、sheet 路由、执行链路展示、明细表、图表、结构化回答
...
1. 在现有单 sheet 路由基础上，先补齐 workbook 内多 sheet 问题的识别、澄清与拆解能力，例如判断用户是否同时引用了多个 sheet、提示用户选择分析范围，或把问题拆成多个可执行步骤；在没有稳定关系建模之前，不直接开放自由 join。
